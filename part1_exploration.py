...
def date_range_for_top_metrics(source):
    # 1) Get top 10 metrics for this source
    top_query = """
    ...
    """
    top_df = pd.read_sql(top_query, engine, params=[source])

    # If no metrics, just return empty DataFrame
    if top_df.empty:
        return top_df

    metrics = top_df["metric"].tolist()

    # 2) Build an IN list with placeholders for each metric
    ...
    # First param is data_source, rest are the metric names
    params = [source] + metrics
    return pd.read_sql(range_query, engine, params=params)
